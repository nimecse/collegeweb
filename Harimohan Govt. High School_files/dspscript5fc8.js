...
(function ($) {
	...
	$.DspThemeJs.prototype = {
		...
		addactions : function() {
			...
			fw.scope.on('DspWidthChanged',function(e){
				fw.RefreshOnWidthChange(e.content);
				$(window).trigger('resize');
			});

			// Refresh events on new content
			fw.scope.on('DspNewContent',function(e){
				fw.refresh_events( e.content );
			});

			// Refresh events on new content
			fw.scope.on('DspBeforePlaceholderReplace',function(e){
				...
				"mouseenter.dsp": function () {
					...
					if ( $submenu.length > 0 ) {
						...
						if ((left_offset + width) > pagewidth) {
							$submenu.addClass('dsp_menu_on_left');
						}
					}
				},
				"mouseleave.dsp": function () {
					var $submenu = $(this).children('ul').first();
					$submenu.removeClass('dsp_menu_on_left');
				}
			});
		},
		...
		enable_fitvids : function ( scope ) {
			...
			var element = scope.find('.dsp_iframe_wrap, .dsp_pb_wrapper');
		...
		{
			var fw = this,
			element = (scope) ? scope.find('.dsp_contact_form_container > form') : $('.dsp_contact_form_container > form');
			...
			element.on( 'submit', function(e){
				...
				var form = $(this),
					response_container = form.find('.dsp_contact_ajax_response:eq(0)'),
					has_error   = false,
					inputs =
					{
					...
					submit_button = form.find('.dsp_contact_submit');

				// FADE THE BUTTON
				submit_button.addClass('dsp_form_loading');
				...
				inputs.fields.each(function()
				{
					...
					p_container.removeClass('dsp_field_not_valid');

					// Check fields that needs to be filled
					if ( field.hasClass('dsp_validate_not_empty') ) {
						if ( field.val() === '' )
						{
							p_container.addClass('dsp_field_not_valid');
							has_error = true;
						}
					}
					else if ( field.hasClass('dsp_validate_is_email') ) {
				 ...
				 //    	//console.log( grecaptcha.getResponse() );
					...
					// 	{
					// 		response_container.html( dsp_contact_form.captcha_not_filled );
					// 		has_error = true;
					// 	}
					// }
				...
				if ( has_error )
				{
					submit_button.removeClass('dsp_form_loading');
		...
		*/
		...
		enable_blog_isotope : function( scope ){
			var elements = scope.find( '.dsp_blog_columns' );
		...
		enable_responsive_menu : function(){
			...
			var page_wrapper = $('#page_wrapper'),
				responsive_trigger = $('.dsp-res-trigger'),
				menu_activated = false,
				back_text = '<li class="dsp_res_menu_go_back"><span class="dsp_res_back_icon glyphicon glyphicon-chevron-left"></span><a href="#">'+DspThemeAjax.dsp_back_text+'</a></li>',
				cloned_menu = $('#main-menu > ul').clone().attr({id:"dsp-res-menu", "class":""});
			...
			var start_responsive_menu = function()
			{
				...
				responsive_trigger.click(function(e){
					e.preventDefault();
					responsive_menu.addClass('dsp-menu-visible');
					set_height();
				});

				// Close the menu when a link is clicked
				responsive_menu.find( 'a' ).on('click',function(e){
					$( '.dsp_res_menu_go_back' ).first().trigger( 'click' );
				});

				// ADD ARROWS TO SUBMENUS TRIGGERS
				responsive_menu
					.find('li:has(> ul.sub-menu), li:has(> div.dsp_mega_container)')
					.addClass('dsp_res_has_submenu')
					.prepend('<span class="dsp_res_submenu_trigger glyphicon glyphicon-chevron-right"></span>');
				// ADD BACK BUTTONS
				responsive_menu
					.find('.dsp_res_has_submenu > ul.sub-menu, .dsp_res_has_submenu > div.dsp_mega_container')
					.addBack()
					.prepend(back_text);

				// REMOVE BACK BUTTON LINK
				$( '.dsp_res_menu_go_back' ).click(function(e){
					e.preventDefault();
					var active_menu = $(this).closest('.dsp-menu-visible');
					active_menu.removeClass('dsp-menu-visible');
					set_height();
					if( active_menu.is('#dsp-res-menu') ) {
						page_wrapper.css({'height':'auto'});
					}
				});

				// OPEN SUBMENU'S ON CLICK
				$('.dsp_res_submenu_trigger').on('click',function(e){
					e.preventDefault();
					$(this).siblings('ul,.dsp_mega_container').addClass('dsp-menu-visible');
					set_height();
				});
			};

			var set_height = function(){
				var _menu = $('.dsp-menu-visible').last(),
			...
			$( window ).on( 'debouncedresize' , function(){
				if ( $(window).width() < DspThemeAjax.res_menu_trigger ) {
					if ( !menu_activated ){
						start_responsive_menu();
						menu_activated = true;
					}
					page_wrapper.addClass('dsp_res_menu_visible');
				}
				else{
					// WE SHOULD HIDE THE MENU
					$('.dsp-menu-visible').removeClass('dsp-menu-visible');
					page_wrapper.css({'height':'auto'}).removeClass('dsp_res_menu_visible');
				}
		...
		enable_general_carousel : function( content ){
			var elements = content.find('.dsp_general_carousel'),
				fw = this;
						...
						var defaults = {
							...
							, pagination: {
								...
								anchorBuilder: function(nr, item) {
									var thumb = '';
									if( $el.is("[data-thumbs]") && $el.data('thumbs') == 'dsp_has_thumbs' ){
		...
		enable_laptop_slider : function( content ){
			...
			var elements = content.find('.dsp_laptop_slider');
		...
		enable_portfolio_sortable : function( content )
		{
			...
			jQuery(function($){
				...
				wpkdspSelector = $("ul#thumbs");
				if(wpkdspSelector && wpkdspSelector.length > 0)
				{
					wpkdspSelector.imagesLoaded( function() {
						wpkdspSelector.isotope({
							itemSelector: ".item",
							animationEngine: "jquery",
							animationOptions: {
								duration: 250,
					...
					if(a_elements && a_elements.length > 0) {
						$.each(a_elements, function (a, b) {
							$(b).on('click', function (w) {
								...
								wpkdspSelector.isotope({filter: $(b).data('filter')});
								wpkdspSelector.isotope('updateSortData').isotope();
							});
						});
					}
					...
					if(b_elements && b_elements.length > 0){
						...
						$.each(b_elements, function(index, element) {
							...
							wpkdspSelector.isotope({sortBy: csb});
							wpkdspSelector.isotope('updateSortData').isotope();
					...
					if(c_elements && c_elements.length > 0) {
						...
						$.each(c_elements,function(index, element) {
							...
							wpkdspSelector.isotope({sortAscending: sortAscending});
							wpkdspSelector.isotope('updateSortData').isotope();
							...
							t.on('click', function(){
								...
								csv = t.data('optionValue');
								sortAscending = csv;
								wpkdspSelector.isotope({sortAscending: csv});
								wpkdspSelector.isotope('updateSortData').isotope();
							});
						});
					}
		...
		enable_screenshoot_box : function( content ){
			var elements = content.find('.dsp_screenshot-carousel');
			if(elements && elements.length){
			    $.each(elements, function(i, e){
			        var self = $(e),
			...
			if(element && element.length){
			    element.on('click', function(e) {
			        ...
			        var self = $(this),
			            ...
			            result_placeholder = self.parent().next('span.dsp_mailchimp_result');
			        $.ajax({
			            url: ajax_url,
			            type: 'POST',
			            data: {
			                dsp_mc_email: self.prevAll('.nl-email').val(),
			                dsp_mailchimp_list: self.prev('.nl-lid').val(),
			                dsp_ajax: '' // Change here with something different
			            },
		...
		enable_testimonial_slider : function( content ){
			var elements = content.find('.dsp_testimonials_carousel');
			if(elements && elements.length){
			    $.each(elements, function(i, e){
			        var self = $(e),
		...
		enable_shop_limited_offers : function( content ){
			var elements = content.find('.dsp_limited_offers');
			if(elements && elements.length){
			    $.each(elements, function(i, e){
			        var self = $(e);
			        // var speed = $(e).data("speed");
...
(function ($)
{
	...
	$(function($)
	{
		...
		$('body').bind('added_to_cart',function (evt,ret) {
			...
			if(mycart && mycart.length > 0) {
				var mycartTop = mycart.offset().top,
					...
					placeholderdiv = $('<div class="popupaddcart">'+dsp_do_login.add_to_cart_text+'</div>');

				$('body').append(placeholderdiv);
				$(placeholderdiv).hide();
				$(placeholderdiv).fadeIn('slow', 'easeInOutExpo',function() {
					//console.log( $(this) );
					var dsp_pos_top = $(this).offset().top,
						dsp_pos_left = $(this).offset().left;
					$(this)
						.css({margin:0,left:dsp_pos_left,top:dsp_pos_top,position:'absolute'})
						.delay(800)
						.animate(
						{
							top: mycartTop,
		...
		// LOGIN FORM
		var dsp_form_login = $('.dsp_form_login');
		dsp_form_login.on('submit', function(event){
			event.preventDefault();

			var form = $(this),
				warning = false,
				button = $('.dsp_sub_button', this),
			...
			if (warning) {
				button.removeClass('dsp_blocked');
				return false;
			}

			if (button.hasClass('dsp_blocked')) {
				return false;
			}

			button.addClass('dsp_blocked');

			$.post(dsp_do_login.ajaxurl, values, function (resp)
			{
				...
				if ($('#login_error', data).length) {
					...
					if ($('.dsp_login_redirect', form).length > 0) {
						$.magnificPopup.close();
						window.location = $('.dsp_login_redirect', form).val();
					}
				}
				button.removeClass('dsp_blocked');
			});
		...
		// LOST PASSWORD
		var dsp_form_lost_pass = $('.dsp_form_lost_pass');
		dsp_form_lost_pass.on('click', function(){
			event.preventDefault();
		...
		if ( typeof DspWooCommerce != 'undefined' ){
			if ( DspWooCommerce.show_thumb_on_hover ){
				...
				$('.single_product_main_image').hover(function(){
						$('.thumbnails',this).find('a').hover(function(el){
							...
							$( '.woocommerce-main-image img' ).attr( 'src', photo_fullsize ).css({'display':'inline-block'});
							$( '.woocommerce-main-image' ).css({width:334,height:334, overflow:'hidden', 'line-height':'334px'});
						}) ;

				},
				function(){
					$( '.woocommerce-main-image img' ).attr( 'src', wcih_main_imgage ).removeAttr('style');
		...
		$('.dsp_pb_editor_enabled .toggle-header').on('click', function(e){
			e.preventDefault();
